...
// Creates a ready-to-view group with Riya, Meera and Kavita already submitted.
export async function POST() {
  const store = getStore();
  const [riya, ...others] = DEMO_MEMBERS;
  const { group, member } = await store.createGroup({ name: "Riya, Meera & Kavita's flat hunt", expectedSize: 3, coordinatorName: riya.name });
